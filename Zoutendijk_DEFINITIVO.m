...
x0 = [4,2];       %Punto desde donde arrancar el método, ha de ser factible (notación [x,y] )            ###########
...
f    = symfun(     x(1)*x(2)+x(1)             ,x);     %La x tras la coma indica qué son las variables            ###########

g(1) = symfun(     x(1)^2+x(2)^2-32            ,x);     %El usuario solo ha de modificar las funciones             ###########
g(2) = symfun(     -x(1)^2+4*x(2)       ,x);     %Las variables se introduciran como                        ###########
g(3) = symfun(     -x(1)+2*x(2)-4                   ,x);     %elementos de un vector x, es decir, de la forma x(i)      ########### 
